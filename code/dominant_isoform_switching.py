...
dominant_df = results_df[results_df["is_dominant"] == True]
print(f"\nAfter filtering to dominant isoforms only:")
print(f"Gene-sample pairs: {len(dominant_df)}")
print(f"Unique genes: {dominant_df['gene_id'].nunique()}")

# ============================================
# CALCULATE SWITCHING
# ============================================

q2 = dominant_df.groupby("gene_id").agg(
...
ax.set_xlabel("Number of Unique Dominant Isoforms Across 18 Samples", fontsize=13)
ax.set_ylabel("Number of Genes", fontsize=13)
ax.set_title("Distribution of Dominant Isoform Switching\nAcross Protein-Coding Genes (No TPM Filter)",
             fontsize=14)
...
output_path = os.path.expanduser(
    "~/finalproject/results/plots/plot_dominant_switching_distribution_no_filter.png")
